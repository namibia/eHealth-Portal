...
/**
 * Ehealth_portal Planning_type Model
 */
class Ehealth_portalModelPlanning_type extends JModelAdmin
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_ehealth_portal.planning_type';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'planning_type', $prefix = 'Ehealth_portalTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_ehealth_portal.planning_type');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		// Get the form.
		$form = $this->loadForm('com_ehealth_portal.planning_type', 'planning_type', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_ehealth_portal.planning_type.' . (int) $id))
		...
		if ($id != 0 && (!$user->authorise('planning_type.edit.name', 'com_ehealth_portal.planning_type.' . (int) $id))
			|| ($id == 0 && !$user->authorise('planning_type.edit.name', 'com_ehealth_portal')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('planning_type.edit.description', 'com_ehealth_portal.planning_type.' . (int) $id))
			|| ($id == 0 && !$user->authorise('planning_type.edit.description', 'com_ehealth_portal')))
		{
			// Disable fields for display.
	...
	{
		return 'media/com_ehealth_portal/js/planning_type.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_ehealth_portal.planning_type.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('core.edit.state', 'com_ehealth_portal.planning_type.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_ehealth_portal.planning_type.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__ehealth_portal_planning_type'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_ehealth_portal.edit.planning_type.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// run the perprocess of the data
			$this->preprocessData('com_ehealth_portal.planning_type', $data);
		}
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				$table = JTable::getInstance('planning_type', 'ehealth_portalTable');

				if ($table->load(array('alias' => $data['alias'])) && ($table->id != $data['id'] || $data['id'] == 0))
				{
					$msg = JText::_('COM_EHEALTH_PORTAL_PLANNING_TYPE_SAVE_WARNING');
				}
