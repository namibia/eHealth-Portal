...
/**
 * Ehealth_portal Referral Model
 */
class Ehealth_portalModelReferral extends JModelAdmin
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_ehealth_portal.referral';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'referral', $prefix = 'Ehealth_portalTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_ehealth_portal.referral');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		// Get the form.
		$form = $this->loadForm('com_ehealth_portal.referral', 'referral', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_ehealth_portal.referral.' . (int) $id))
		...
		if ($id != 0 && (!$user->authorise('referral.edit.name', 'com_ehealth_portal.referral.' . (int) $id))
			|| ($id == 0 && !$user->authorise('referral.edit.name', 'com_ehealth_portal')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('referral.edit.description', 'com_ehealth_portal.referral.' . (int) $id))
			|| ($id == 0 && !$user->authorise('referral.edit.description', 'com_ehealth_portal')))
		{
			// Disable fields for display.
	...
	{
		return 'media/com_ehealth_portal/js/referral.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_ehealth_portal.referral.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('core.edit.state', 'com_ehealth_portal.referral.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_ehealth_portal.referral.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_ehealth_portal.edit.referral.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// run the perprocess of the data
			$this->preprocessData('com_ehealth_portal.referral', $data);
		}
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo			= Ehealth_portalHelper::getActions('referral');
		$this->batchSet			= true;
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				$table = JTable::getInstance('referral', 'ehealth_portalTable');

				if ($table->load(array('alias' => $data['alias'])) && ($table->id != $data['id'] || $data['id'] == 0))
				{
					$msg = JText::_('COM_EHEALTH_PORTAL_REFERRAL_SAVE_WARNING');
				}
