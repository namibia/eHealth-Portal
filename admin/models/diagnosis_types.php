...
/**
 * Diagnosis_types Model
 */
class Ehealth_portalModelDiagnosis_types extends JModelList
{
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = Ehealth_portalHelper::checkArray($pks)) !== false || 'bulk' === $pks)
		{
			...
			// From the ehealth_portal_diagnosis_type table
			$query->from($db->quoteName('#__ehealth_portal_diagnosis_type', 'a'));
	...
	*/
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__ehealth_portal_diagnosis_type'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__ehealth_portal_diagnosis_type'))->set($fields)->where($conditions); 
