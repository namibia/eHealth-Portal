...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		immunisationvaccinetypes.php
...
/**
 * Immunisationvaccinetypes Form Field class for the Ehealth_portal component
 */
class JFormFieldImmunisationvaccinetypes extends JFormFieldList
{
	/**
	...
	protected function getInput()
	{
		...
		if ($set_button === 'true')
		{
			...
			// only add if user allowed to create immunisation_vaccine_type
			if ($user->authorise('core.create', 'com_ehealth_portal') && $app->isAdmin()) // TODO for now only in admin area.
			{
				...
				$button[] = '<a id="'.$button_code_name.'Create" class="btn btn-small btn-success hasTooltip" title="'.JText::sprintf('COM_EHEALTH_PORTAL_CREATE_NEW_S', $button_label).'" style="border-radius: 0px 4px 4px 0px; padding: 4px 4px 4px 7px;"
					href="index.php?option=com_ehealth_portal&amp;view=immunisation_vaccine_type&amp;layout=edit'.$ref.'" >
					<span class="icon-new icon-white"></span></a>';
			}
			// only add if user allowed to edit immunisation_vaccine_type
				...
				$script[] = "
					...
					function ".$button_code_name."Button(value) {
						if (value > 0) {
							...
							var url = 'index.php?option=com_ehealth_portal&view=immunisation_vaccine_types&task=immunisation_vaccine_type.edit&id='+value+'".$refJ."';
			...
			// check if button was created for immunisation_vaccine_type field.
	...
	protected function getOptions()
	{
		...
		if ($items)
		{
			...
			{
				$options[] = JHtml::_('select.option', $item->id, $item->immunisation_vaccine_type_name);
			}
		}
		return $options;
	}
