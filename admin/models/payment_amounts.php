...
/**
 * Payment_amounts Model
 */
class Ehealth_portalModelPayment_amounts extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__ehealth_portal_payment_amount', 'a'));
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = Ehealth_portalHelper::checkArray($pks)) !== false || 'bulk' === $pks)
		{
			...
			// From the ehealth_portal_payment_amount table
			$query->from($db->quoteName('#__ehealth_portal_payment_amount', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__ehealth_portal_payment_amount");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__ehealth_portal_payment_amount'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__ehealth_portal_payment_amount'))->set($fields)->where($conditions); 
