...
/**
 * Immunisation_types Model
 */
class Ehealth_portalModelImmunisation_types extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__ehealth_portal_immunisation_type', 'a'));
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = Ehealth_portalHelper::checkArray($pks)) !== false || 'bulk' === $pks)
		{
			...
			// From the ehealth_portal_immunisation_type table
			$query->from($db->quoteName('#__ehealth_portal_immunisation_type', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__ehealth_portal_immunisation_type");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__ehealth_portal_immunisation_type'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__ehealth_portal_immunisation_type'))->set($fields)->where($conditions); 
