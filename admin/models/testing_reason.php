...
/**
 * Ehealth_portal Testing_reason Model
 */
class Ehealth_portalModelTesting_reason extends JModelAdmin
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_ehealth_portal.testing_reason';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'testing_reason', $prefix = 'Ehealth_portalTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_ehealth_portal.testing_reason');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		// Get the form.
		$form = $this->loadForm('com_ehealth_portal.testing_reason', 'testing_reason', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_ehealth_portal.testing_reason.' . (int) $id))
		...
		if ($id != 0 && (!$user->authorise('testing_reason.edit.name', 'com_ehealth_portal.testing_reason.' . (int) $id))
			|| ($id == 0 && !$user->authorise('testing_reason.edit.name', 'com_ehealth_portal')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('testing_reason.edit.description', 'com_ehealth_portal.testing_reason.' . (int) $id))
			|| ($id == 0 && !$user->authorise('testing_reason.edit.description', 'com_ehealth_portal')))
		{
			// Disable fields for display.
	...
	{
		return 'media/com_ehealth_portal/js/testing_reason.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_ehealth_portal.testing_reason.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('core.edit.state', 'com_ehealth_portal.testing_reason.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_ehealth_portal.testing_reason.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_ehealth_portal.edit.testing_reason.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// run the perprocess of the data
			$this->preprocessData('com_ehealth_portal.testing_reason', $data);
		}
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo			= Ehealth_portalHelper::getActions('testing_reason');
		$this->batchSet			= true;
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				$table = JTable::getInstance('testing_reason', 'ehealth_portalTable');

				if ($table->load(array('alias' => $data['alias'])) && ($table->id != $data['id'] || $data['id'] == 0))
				{
					$msg = JText::_('COM_EHEALTH_PORTAL_TESTING_REASON_SAVE_WARNING');
				}
