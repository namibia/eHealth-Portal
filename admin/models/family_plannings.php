...
/**
 * Family_plannings Model
 */
class Ehealth_portalModelFamily_plannings extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__ehealth_portal_family_planning', 'a'));

		// From the ehealth_portal_planning_type table.
		$query->select($db->quoteName('g.name','diagnosis_name'));
		$query->join('LEFT', $db->quoteName('#__ehealth_portal_planning_type', 'g') . ' ON (' . $db->quoteName('a.diagnosis') . ' = ' . $db->quoteName('g.id') . ')');
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = Ehealth_portalHelper::checkArray($pks)) !== false || 'bulk' === $pks)
		{
			...
			// From the ehealth_portal_family_planning table
			$query->from($db->quoteName('#__ehealth_portal_family_planning', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__ehealth_portal_family_planning");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__ehealth_portal_family_planning'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__ehealth_portal_family_planning'))->set($fields)->where($conditions); 
