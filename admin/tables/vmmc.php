 ...
 * Vmmcs Table class
 */
class Ehealth_portalTableVmmc extends JTable
{
	/**
	...
	{
		parent::__construct('#__ehealth_portal_vmmc', 'id', $db);

		// Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_ehealth_portal.vmmc'));
	}	
	...
	public function store($updateNulls = false)
	{
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('vmmc', 'Ehealth_portalTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_EHEALTH_PORTAL_VMMC_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealth_portal.vmmc.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_ehealth_portal.vmmc.'.(int) $this->$k;
	}
