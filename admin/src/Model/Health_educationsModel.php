...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Health_educationsModel.php
 ...
 */
class Health_educationsModel extends ListModel
{
	public function __construct($config = [])
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'a.education_type','education_type',
				'a.patient','patient',
				'g.name','health_education_topic'
			);
		}
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$education_type = $this->getUserStateFromRequest($this->context . '.filter.education_type', 'filter_education_type');
		if ($formSubmited)
		{
			$education_type = $app->input->post->get('education_type');
			$this->setState('filter.education_type', $education_type);
		}
		...
		$health_education_topic = $this->getUserStateFromRequest($this->context . '.filter.health_education_topic', 'filter_health_education_topic');
		if ($formSubmited)
		{
			$health_education_topic = $app->input->post->get('health_education_topic');
			$this->setState('filter.health_education_topic', $health_education_topic);
		}
	...
	{
		// Array of education_type language strings
	...
	protected function getListQuery()
	{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.health_education_topic LIKE '.$search.' OR g.name LIKE '.$search.' OR a.patient LIKE '.$search.')');
			}
		}

		// Filter by Education_type.
		$_education_type = $this->getState('filter.education_type');
		if (is_numeric($_education_type))
		{
			if (is_float($_education_type))
			{
				$query->where('a.education_type = ' . (float) $_education_type);
			}
			else
			{
		...
		elseif (EhealthportalHelper::checkArray($_patient))
		{
			...
			// Filter by the Patient Array.
			$query->where('a.patient IN (' . implode(',', $_patient) . ')');
		}
	...
	*/
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':' . $this->getState('filter.health_education_topic');
