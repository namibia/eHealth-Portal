...
/**
 * Ehealthportal Unit Admin Model
 */
class UnitModel extends AdminModel
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_ehealthportal.unit';
	 ...
	 * @throws  \Exception
	 */
	public function getTable($type = 'unit', $prefix = 'Administrator', $config = [])
	{
	...
	public function getForm($data = [], $loadData = true, $options = ['control' => 'jform'])
	{
		...
		// Get the form.
		$form = $this->loadForm('com_ehealthportal.unit', 'unit', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_ehealthportal.unit.' . (int) $id))
		...
		if ($id != 0 && (!$user->authorise('unit.edit.name', 'com_ehealthportal.unit.' . (int) $id))
			|| ($id == 0 && !$user->authorise('unit.edit.name', 'com_ehealthportal')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('unit.edit.description', 'com_ehealthportal.unit.' . (int) $id))
			|| ($id == 0 && !$user->authorise('unit.edit.description', 'com_ehealthportal')))
		{
			// Disable fields for display.
	...
	{
		return 'media/com_ehealthportal/js/unit.js';
	}
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('core.delete', 'com_ehealthportal.unit.' . (int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('core.edit.state', 'com_ehealthportal.unit.' . (int) $recordId);
	...
	protected function allowEdit($data = [], $key = 'id')
	{
		...
		return Factory::getApplication()->getIdentity()->authorise('core.edit', 'com_ehealthportal.unit.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__ehealthportal_unit'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_ehealthportal.edit.unit.data', []);
		...
		$this->preprocessData('com_ehealthportal.unit', $data);
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo = EhealthportalHelper::getActions('unit');
		$this->batchSet = true;
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('core.create') && !$this->canDo->get('unit.batch'))
		{
			return false;
		}
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('core.edit') && !$this->canDo->get('unit.batch'))
		{
	...
	public function save($data)
	{
		...
		if (empty($data['guid']) && $data['id'] > 0)
		{
			...
			$data['guid'] = (string) EhealthportalHelper::getVar('unit', $data['id'], 'id', 'guid');
		}
		// Set the GUID if empty or not valid
		while (!EhealthportalHelper::validGUID($data['guid'], "unit", $data['id']))
		{
			// must always be set
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				{
					$msg = Text::_('COM_EHEALTHPORTAL_UNIT_SAVE_WARNING');
				}
