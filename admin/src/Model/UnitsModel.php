...
class UnitsModel extends ListModel
{
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = UtilitiesArrayHelper::check($pks)) !== false || 'bulk' === $pks)
		{
			...
			// From the ehealthportal_unit table
			$query->from($db->quoteName('#__ehealthportal_unit', 'a'));
	...
	*/
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__ehealthportal_unit'));
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__ehealthportal_unit'))->set($fields)->where($conditions); 
