...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Planning_typeModel.php
...
/**
 * Ehealthportal Planning_type Admin Model
 */
class Planning_typeModel extends AdminModel
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_ehealthportal.planning_type';
	 ...
	 * @throws  \Exception
	 */
	public function getTable($type = 'planning_type', $prefix = 'Administrator', $config = [])
	{
	...
	public function getForm($data = [], $loadData = true, $options = ['control' => 'jform'])
	{
		...
		// Get the form.
		$form = $this->loadForm('com_ehealthportal.planning_type', 'planning_type', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_ehealthportal.planning_type.' . (int) $id))
		...
		if ($id != 0 && (!$user->authorise('planning_type.edit.name', 'com_ehealthportal.planning_type.' . (int) $id))
			|| ($id == 0 && !$user->authorise('planning_type.edit.name', 'com_ehealthportal')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('planning_type.edit.description', 'com_ehealthportal.planning_type.' . (int) $id))
			|| ($id == 0 && !$user->authorise('planning_type.edit.description', 'com_ehealthportal')))
		{
			// Disable fields for display.
	...
	{
		return 'media/com_ehealthportal/js/planning_type.js';
	}
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('core.delete', 'com_ehealthportal.planning_type.' . (int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('core.edit.state', 'com_ehealthportal.planning_type.' . (int) $recordId);
	...
	protected function allowEdit($data = [], $key = 'id')
	{
		...
		return Factory::getApplication()->getIdentity()->authorise('core.edit', 'com_ehealthportal.planning_type.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__ehealthportal_planning_type'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_ehealthportal.edit.planning_type.data', []);
		...
		$this->preprocessData('com_ehealthportal.planning_type', $data);
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo = EhealthportalHelper::getActions('planning_type');
		$this->batchSet = true;
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('core.create') && !$this->canDo->get('planning_type.batch'))
		{
			return false;
		}
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('core.edit') && !$this->canDo->get('planning_type.batch'))
		{
	...
	public function save($data)
	{
		...
		if (empty($data['guid']) && $data['id'] > 0)
		{
			...
			$data['guid'] = (string) EhealthportalHelper::getVar('planning_type', $data['id'], 'id', 'guid');
		}
		// Set the GUID if empty or not valid
		while (!EhealthportalHelper::validGUID($data['guid'], "planning_type", $data['id']))
		{
			// must always be set
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				{
					$msg = Text::_('COM_EHEALTHPORTAL_PLANNING_TYPE_SAVE_WARNING');
				}
