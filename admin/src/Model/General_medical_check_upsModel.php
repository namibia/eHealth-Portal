...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		General_medical_check_upsModel.php
...
/**
 * General_medical_check_ups List Model
 */
class General_medical_check_upsModel extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__ehealthportal_general_medical_check_up', 'a'));

		// From the ehealthportal_diagnosis_type table.
		$query->select($db->quoteName('g.name','diagnosis_name'));
		$query->join('LEFT', $db->quoteName('#__ehealthportal_diagnosis_type', 'g') . ' ON (' . $db->quoteName('a.diagnosis') . ' = ' . $db->quoteName('g.id') . ')');
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = UtilitiesArrayHelper::check($pks)) !== false || 'bulk' === $pks)
		{
			...
			// From the ehealthportal_general_medical_check_up table
			$query->from($db->quoteName('#__ehealthportal_general_medical_check_up', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = Factory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__ehealthportal_general_medical_check_up");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__ehealthportal_general_medical_check_up'))->set($fields)->where($conditions); 
