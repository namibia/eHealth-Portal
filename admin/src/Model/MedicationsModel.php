...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		MedicationsModel.php
...
/**
 * Medications List Model
 */
class MedicationsModel extends ListModel
{
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = UtilitiesArrayHelper::check($pks)) !== false || 'bulk' === $pks)
		{
			...
			// From the ehealthportal_medication table
			$query->from($db->quoteName('#__ehealthportal_medication', 'a'));
	...
	*/
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__ehealthportal_medication'));
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__ehealthportal_medication'))->set($fields)->where($conditions); 
