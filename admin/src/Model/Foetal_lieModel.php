...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Foetal_lieModel.php
...
/**
 * Ehealthportal Foetal_lie Admin Model
 */
class Foetal_lieModel extends AdminModel
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_ehealthportal.foetal_lie';
	 ...
	 * @throws  \Exception
	 */
	public function getTable($type = 'foetal_lie', $prefix = 'Administrator', $config = [])
	{
	...
	public function getForm($data = [], $loadData = true, $options = ['control' => 'jform'])
	{
		...
		// Get the form.
		$form = $this->loadForm('com_ehealthportal.foetal_lie', 'foetal_lie', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_ehealthportal.foetal_lie.' . (int) $id))
		...
		if ($id != 0 && (!$user->authorise('foetal_lie.edit.name', 'com_ehealthportal.foetal_lie.' . (int) $id))
			|| ($id == 0 && !$user->authorise('foetal_lie.edit.name', 'com_ehealthportal')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('foetal_lie.edit.description', 'com_ehealthportal.foetal_lie.' . (int) $id))
			|| ($id == 0 && !$user->authorise('foetal_lie.edit.description', 'com_ehealthportal')))
		{
			// Disable fields for display.
	...
	{
		return 'media/com_ehealthportal/js/foetal_lie.js';
	}
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('core.delete', 'com_ehealthportal.foetal_lie.' . (int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('core.edit.state', 'com_ehealthportal.foetal_lie.' . (int) $recordId);
	...
	protected function allowEdit($data = [], $key = 'id')
	{
		...
		return Factory::getApplication()->getIdentity()->authorise('core.edit', 'com_ehealthportal.foetal_lie.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__ehealthportal_foetal_lie'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_ehealthportal.edit.foetal_lie.data', []);
		...
		$this->preprocessData('com_ehealthportal.foetal_lie', $data);
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo = EhealthportalHelper::getActions('foetal_lie');
		$this->batchSet = true;
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('core.create') && !$this->canDo->get('foetal_lie.batch'))
		{
			return false;
		}
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('core.edit') && !$this->canDo->get('foetal_lie.batch'))
		{
	...
	public function save($data)
	{
		...
		if (empty($data['guid']) && $data['id'] > 0)
		{
			...
			$data['guid'] = (string) EhealthportalHelper::getVar('foetal_lie', $data['id'], 'id', 'guid');
		}
		// Set the GUID if empty or not valid
		while (!EhealthportalHelper::validGUID($data['guid'], "foetal_lie", $data['id']))
		{
			// must always be set
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				{
					$msg = Text::_('COM_EHEALTHPORTAL_FOETAL_LIE_SAVE_WARNING');
				}
