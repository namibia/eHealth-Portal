...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Family_planningsModel.php
...
/**
 * Family_plannings List Model
 */
class Family_planningsModel extends ListModel
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$diagnosis = $this->getUserStateFromRequest($this->context . '.filter.diagnosis', 'filter_diagnosis');
		if ($formSubmited)
		{
			$diagnosis = $app->input->post->get('diagnosis');
			$this->setState('filter.diagnosis', $diagnosis);
		}
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__ehealthportal_family_planning', 'a'));

		// From the ehealthportal_planning_type table.
		$query->select($db->quoteName('g.name','diagnosis_name'));
		$query->join('LEFT', $db->quoteName('#__ehealthportal_planning_type', 'g') . ' ON (' . $db->quoteName('a.diagnosis') . ' = ' . $db->quoteName('g.id') . ')');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.patient LIKE '.$search.' OR a.diagnosis LIKE '.$search.' OR g.name LIKE '.$search.')');
			}
		}
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = UtilitiesArrayHelper::check($pks)) !== false || 'bulk' === $pks)
		{
			...
			// From the ehealthportal_family_planning table
			$query->from($db->quoteName('#__ehealthportal_family_planning', 'a'));
			...
			if ($export_text_only)
			{
				...
				// From the ehealthportal_planning_type table.
				$query->select($db->quoteName('g.name','diagnosis'));
				$query->join('LEFT', $db->quoteName('#__ehealthportal_planning_type', 'g') . ' ON (' . $db->quoteName('a.diagnosis') . ' = ' . $db->quoteName('g.id') . ')');
			}
			// Implement View Level Access
			if (!$user->authorise('core.options', 'com_ehealthportal'))
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = Factory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__ehealthportal_family_planning");
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':' . $this->getState('filter.diagnosis');
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__ehealthportal_family_planning'))->set($fields)->where($conditions); 
