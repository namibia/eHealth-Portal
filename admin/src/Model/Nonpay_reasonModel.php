...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Nonpay_reasonModel.php
...
/**
 * Ehealthportal Nonpay_reason Admin Model
 */
class Nonpay_reasonModel extends AdminModel
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_ehealthportal.nonpay_reason';
	 ...
	 * @throws  \Exception
	 */
	public function getTable($type = 'nonpay_reason', $prefix = 'Administrator', $config = [])
	{
	...
	public function getForm($data = [], $loadData = true, $options = ['control' => 'jform'])
	{
		...
		// Get the form.
		$form = $this->loadForm('com_ehealthportal.nonpay_reason', 'nonpay_reason', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_ehealthportal.nonpay_reason.' . (int) $id))
		...
		if ($id != 0 && (!$user->authorise('nonpay_reason.edit.name', 'com_ehealthportal.nonpay_reason.' . (int) $id))
			|| ($id == 0 && !$user->authorise('nonpay_reason.edit.name', 'com_ehealthportal')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('nonpay_reason.edit.description', 'com_ehealthportal.nonpay_reason.' . (int) $id))
			|| ($id == 0 && !$user->authorise('nonpay_reason.edit.description', 'com_ehealthportal')))
		{
			// Disable fields for display.
	...
	{
		return 'media/com_ehealthportal/js/nonpay_reason.js';
	}
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('core.delete', 'com_ehealthportal.nonpay_reason.' . (int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('core.edit.state', 'com_ehealthportal.nonpay_reason.' . (int) $recordId);
	...
	protected function allowEdit($data = [], $key = 'id')
	{
		...
		return Factory::getApplication()->getIdentity()->authorise('core.edit', 'com_ehealthportal.nonpay_reason.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__ehealthportal_nonpay_reason'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_ehealthportal.edit.nonpay_reason.data', []);
		...
		$this->preprocessData('com_ehealthportal.nonpay_reason', $data);
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo = EhealthportalHelper::getActions('nonpay_reason');
		$this->batchSet = true;
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('core.create') && !$this->canDo->get('nonpay_reason.batch'))
		{
			return false;
		}
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('core.edit') && !$this->canDo->get('nonpay_reason.batch'))
		{
	...
	public function save($data)
	{
		...
		if (empty($data['guid']) && $data['id'] > 0)
		{
			...
			$data['guid'] = (string) EhealthportalHelper::getVar('nonpay_reason', $data['id'], 'id', 'guid');
		}
		// Set the GUID if empty or not valid
		while (!EhealthportalHelper::validGUID($data['guid'], "nonpay_reason", $data['id']))
		{
			// must always be set
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				{
					$msg = Text::_('COM_EHEALTHPORTAL_NONPAY_REASON_SAVE_WARNING');
				}
