...
/**
 * Strengths List Model
 */
class StrengthsModel extends ListModel
{
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = UtilitiesArrayHelper::check($pks)) !== false || 'bulk' === $pks)
		{
			...
			// From the ehealthportal_strength table
			$query->from($db->quoteName('#__ehealthportal_strength', 'a'));
	...
	*/
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__ehealthportal_strength'));
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__ehealthportal_strength'))->set($fields)->where($conditions); 
