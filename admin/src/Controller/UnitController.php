...
class UnitController extends FormController
{
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_EHEALTHPORTAL_UNIT';
	...
	public function __construct(
		...
		$this->view_list = 'Units'; // safeguard for setting the return view listing to the main view.
	}
	...
	protected function allowAdd($data = [])
	{
		...
		// Access check.
		$access = $user->authorise('unit.access', 'com_ehealthportal');
		if (!$access)
		{
			return false;
		}
	...
	protected function allowEdit($data = [], $key = 'id')
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit', 'com_ehealthportal.unit.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('core.edit.own', 'com_ehealthportal.unit.' . $recordId))
				{
	...
	public function batch($model = null)
	{
		...
		// Preset the redirect
		$this->setRedirect(Route::_('index.php?option=com_ehealthportal&view=units' . $this->getRedirectToListAppend(), false));
