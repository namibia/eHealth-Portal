...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		General_medical_check_upsController.php
	...
	public function exportData()
	{
		...
		if ($user->authorise('general_medical_check_up.export', 'com_ehealthportal') && $user->authorise('core.export', 'com_ehealthportal'))
		{
			...
			if (UtilitiesArrayHelper::check($data))
			{
				...
				EhealthportalHelper::xls($data,'General_medical_check_ups_'.$date->format('jS_F_Y'),'General medical check ups exported ('.$date->format('jS F, Y').')','general medical check ups');
			}
		}
		// Redirect to the list screen with error.
		$message = Text::_('COM_EHEALTHPORTAL_EXPORT_FAILED');
		$this->setRedirect(Route::_('index.php?option=com_ehealthportal&view=general_medical_check_ups', false), $message, 'error');
		return;
	}
