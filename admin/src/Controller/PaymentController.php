...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		PaymentController.php
...
class PaymentController extends FormController
{
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_EHEALTHPORTAL_PAYMENT';
	...
	protected function allowAdd($data = [])
	{
		...
		// Access check.
		$access = $user->authorise('payment.access', 'com_ehealthportal');
		if (!$access)
		{
			return false;
		}
	...
	protected function allowEdit($data = [], $key = 'id')
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit', 'com_ehealthportal.payment.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('core.edit.own', 'com_ehealthportal.payment.' . $recordId))
				{
	...
	public function batch($model = null)
	{
		...
		// Preset the redirect
		$this->setRedirect(Route::_('index.php?option=com_ehealthportal&view=payments' . $this->getRedirectToListAppend(), false));
