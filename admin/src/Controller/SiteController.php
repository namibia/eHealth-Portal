...
class SiteController extends FormController
{
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_EHEALTHPORTAL_SITE';
	...
	public function __construct(
		...
		$this->view_list = 'Sites'; // safeguard for setting the return view listing to the main view.
	}
	...
	protected function allowAdd($data = [])
	{
		...
		// Access check.
		$access = $user->authorise('site.access', 'com_ehealthportal');
		if (!$access)
		{
			return false;
		}
	...
	protected function allowEdit($data = [], $key = 'id')
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit', 'com_ehealthportal.site.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('core.edit.own', 'com_ehealthportal.site.' . $recordId))
				{
	...
	public function batch($model = null)
	{
		...
		// Preset the redirect
		$this->setRedirect(Route::_('index.php?option=com_ehealthportal&view=sites' . $this->getRedirectToListAppend(), false));
