...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Health_education_topicTable.php
...
/**
 * Health_education_topics Table class
 */
class Health_education_topicTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.health_education_topic';
		...
		parent::__construct('#__ehealthportal_health_education_topic', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Health_education_topic table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New health_education_topic. A health_education_topic created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_EHEALTHPORTAL_HEALTH_EDUCATION_TOPIC_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_HEALTH_EDUCATION_TOPIC_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.health_education_topic.'.$this->id);
			$this->setRules($rules);
		}
