 ...
 * Units Table class
 */
class UnitTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.unit';
		...
		parent::__construct('#__ehealthportal_unit', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Unit table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New unit. A unit created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_EHEALTHPORTAL_UNIT_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_UNIT_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.unit.'.$this->id);
			$this->setRules($rules);
		}
