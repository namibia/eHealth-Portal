...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Foetal_lieTable.php
...
/**
 * Foetal_lies Table class
 */
class Foetal_lieTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.foetal_lie';
		...
		parent::__construct('#__ehealthportal_foetal_lie', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Foetal_lie table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New foetal_lie. A foetal_lie created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			if ($table->load(['alias' => $this->alias]) && ($table->id != $this->id || $this->id == 0))
			{
				...
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_FOETAL_LIE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.foetal_lie.'.$this->id);
			$this->setRules($rules);
		}
