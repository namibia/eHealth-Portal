 ...
 * Sites Table class
 */
class SiteTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.site';
		...
		parent::__construct('#__ehealthportal_site', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Site table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New site. A site created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_EHEALTHPORTAL_SITE_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_SITE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.site.'.$this->id);
			$this->setRules($rules);
		}
