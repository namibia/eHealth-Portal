...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Nonpay_reasonTable.php
...
/**
 * Nonpay_reasons Table class
 */
class Nonpay_reasonTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.nonpay_reason';
		...
		parent::__construct('#__ehealthportal_nonpay_reason', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Nonpay_reason table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New nonpay_reason. A nonpay_reason created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_EHEALTHPORTAL_NONPAY_REASON_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_NONPAY_REASON_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.nonpay_reason.'.$this->id);
			$this->setRules($rules);
		}
