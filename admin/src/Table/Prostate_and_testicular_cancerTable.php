...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Prostate_and_testicular_cancerTable.php
...
/-----------------------------------------------------------------------------------------------------------------------------*/
namespace JCB\Component\Ehealthportal\Administrator\Table;
...
/**
 * Prostate_and_testicular_cancers Table class
 */
class Prostate_and_testicular_cancerTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.prostate_and_testicular_cancer';
		...
		parent::__construct('#__ehealthportal_prostate_and_testicular_cancer', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Prostate_and_testicular_cancer table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New prostate_and_testicular_cancer. A prostate_and_testicular_cancer created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_EHEALTHPORTAL_PROSTATE_AND_TESTICULAR_CANCER_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_PROSTATE_AND_TESTICULAR_CANCER_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.prostate_and_testicular_cancer.'.$this->id);
			$this->setRules($rules);
		}
	...
	protected function _getAssetParentId(Table $table = null, $id = null)
	{
		...
		// load the ehealthportal asset
		$assets->loadByName('com_ehealthportal');
