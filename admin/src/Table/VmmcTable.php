 ...
 * Vmmcs Table class
 */
class VmmcTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.vmmc';
		...
		parent::__construct('#__ehealthportal_vmmc', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Vmmc table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New vmmc. A vmmc created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_EHEALTHPORTAL_VMMC_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_VMMC_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.vmmc.'.$this->id);
			$this->setRules($rules);
		}
