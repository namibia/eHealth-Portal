...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Antenatal_careTable.php
...
/**
 * Antenatal_cares Table class
 */
class Antenatal_careTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.antenatal_care';
		...
		parent::__construct('#__ehealthportal_antenatal_care', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Antenatal_care table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New antenatal_care. A antenatal_care created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_EHEALTHPORTAL_ANTENATAL_CARE_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_ANTENATAL_CARE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.antenatal_care.'.$this->id);
			$this->setRules($rules);
		}
