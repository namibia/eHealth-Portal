...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		General_medical_check_upTable.php
...
/**
 * General_medical_check_ups Table class
 */
class General_medical_check_upTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.general_medical_check_up';
		...
		parent::__construct('#__ehealthportal_general_medical_check_up', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the General_medical_check_up table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New general_medical_check_up. A general_medical_check_up created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			if ($table->load(['alias' => $this->alias]) && ($table->id != $this->id || $this->id == 0))
			{
				...
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_GENERAL_MEDICAL_CHECK_UP_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.general_medical_check_up.'.$this->id);
			$this->setRules($rules);
		}
