...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Foetal_engagementTable.php
...
/**
 * Foetal_engagements Table class
 */
class Foetal_engagementTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.foetal_engagement';
		...
		parent::__construct('#__ehealthportal_foetal_engagement', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Foetal_engagement table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New foetal_engagement. A foetal_engagement created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_EHEALTHPORTAL_FOETAL_ENGAGEMENT_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_FOETAL_ENGAGEMENT_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.foetal_engagement.'.$this->id);
			$this->setRules($rules);
		}
