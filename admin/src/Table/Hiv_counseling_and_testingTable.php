...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Hiv_counseling_and_testingTable.php
 ...
 */
class Hiv_counseling_and_testingTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.hiv_counseling_and_testing';
		...
		parent::__construct('#__ehealthportal_hiv_counseling_and_testing', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Hiv_counseling_and_testing table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New hiv_counseling_and_testing. A hiv_counseling_and_testing created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_EHEALTHPORTAL_HIV_COUNSELING_AND_TESTING_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_HIV_COUNSELING_AND_TESTING_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.hiv_counseling_and_testing.'.$this->id);
			$this->setRules($rules);
		}
