...
/**
 * Referrals Table class
 */
class ReferralTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.referral';
		...
		parent::__construct('#__ehealthportal_referral', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Referral table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New referral. A referral created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			if ($table->load(['alias' => $this->alias]) && ($table->id != $this->id || $this->id == 0))
			{
				...
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_REFERRAL_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.referral.'.$this->id);
			$this->setRules($rules);
		}
