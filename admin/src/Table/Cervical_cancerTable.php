...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Cervical_cancerTable.php
...
/**
 * Cervical_cancers Table class
 */
class Cervical_cancerTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.cervical_cancer';
		...
		parent::__construct('#__ehealthportal_cervical_cancer', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Cervical_cancer table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New cervical_cancer. A cervical_cancer created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_EHEALTHPORTAL_CERVICAL_CANCER_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_CERVICAL_CANCER_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.cervical_cancer.'.$this->id);
			$this->setRules($rules);
		}
