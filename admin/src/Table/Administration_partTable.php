...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Administration_partTable.php
...
/**
 * Administration_parts Table class
 */
class Administration_partTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.administration_part';
		...
		parent::__construct('#__ehealthportal_administration_part', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Administration_part table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New administration_part. A administration_part created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_EHEALTHPORTAL_ADMINISTRATION_PART_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_ADMINISTRATION_PART_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.administration_part.'.$this->id);
			$this->setRules($rules);
		}
