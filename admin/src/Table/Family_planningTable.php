...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Family_planningTable.php
...
/**
 * Family_plannings Table class
 */
class Family_planningTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.family_planning';
		...
		parent::__construct('#__ehealthportal_family_planning', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Family_planning table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New family_planning. A family_planning created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			if ($table->load(['alias' => $this->alias]) && ($table->id != $this->id || $this->id == 0))
			{
				...
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_FAMILY_PLANNING_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.family_planning.'.$this->id);
			$this->setRules($rules);
		}
