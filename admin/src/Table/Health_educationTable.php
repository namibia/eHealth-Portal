...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Health_educationTable.php
...
/**
 * Health_educations Table class
 */
class Health_educationTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.health_education';
		...
		parent::__construct('#__ehealthportal_health_education', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Health_education table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New health_education. A health_education created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_EHEALTHPORTAL_HEALTH_EDUCATION_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_HEALTH_EDUCATION_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.health_education.'.$this->id);
			$this->setRules($rules);
		}
