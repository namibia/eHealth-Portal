...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		MedicationTable.php
...
/**
 * Medications Table class
 */
class MedicationTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.medication';
		...
		parent::__construct('#__ehealthportal_medication', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Medication table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New medication. A medication created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_EHEALTHPORTAL_MEDICATION_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_MEDICATION_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.medication.'.$this->id);
			$this->setRules($rules);
		}
