...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Breast_cancerTable.php
...
/**
 * Breast_cancers Table class
 */
class Breast_cancerTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.breast_cancer';
		...
		parent::__construct('#__ehealthportal_breast_cancer', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Breast_cancer table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New breast_cancer. A breast_cancer created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			if ($table->load(['alias' => $this->alias]) && ($table->id != $this->id || $this->id == 0))
			{
				...
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_BREAST_CANCER_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.breast_cancer.'.$this->id);
			$this->setRules($rules);
		}
