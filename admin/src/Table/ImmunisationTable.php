...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		ImmunisationTable.php
...
/**
 * Immunisations Table class
 */
class ImmunisationTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.immunisation';
		...
		parent::__construct('#__ehealthportal_immunisation', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Immunisation table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New immunisation. A immunisation created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_EHEALTHPORTAL_IMMUNISATION_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_IMMUNISATION_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.immunisation.'.$this->id);
			$this->setRules($rules);
		}
