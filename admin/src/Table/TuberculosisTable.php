...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		TuberculosisTable.php
...
/**
 * Tuberculoses Table class
 */
class TuberculosisTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.tuberculosis';
		...
		parent::__construct('#__ehealthportal_tuberculosis', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Tuberculosis table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New tuberculosis. A tuberculosis created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_EHEALTHPORTAL_TUBERCULOSIS_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_TUBERCULOSIS_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.tuberculosis.'.$this->id);
			$this->setRules($rules);
		}
