...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Foetal_presentationTable.php
...
/**
 * Foetal_presentations Table class
 */
class Foetal_presentationTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.foetal_presentation';
		...
		parent::__construct('#__ehealthportal_foetal_presentation', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Foetal_presentation table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New foetal_presentation. A foetal_presentation created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_EHEALTHPORTAL_FOETAL_PRESENTATION_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_FOETAL_PRESENTATION_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.foetal_presentation.'.$this->id);
			$this->setRules($rules);
		}
