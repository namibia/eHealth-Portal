...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Immunisation_vaccine_typeTable.php
...
/**
 * Immunisation_vaccine_types Table class
 */
class Immunisation_vaccine_typeTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.immunisation_vaccine_type';
		...
		parent::__construct('#__ehealthportal_immunisation_vaccine_type', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Immunisation_vaccine_type table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New immunisation_vaccine_type. A immunisation_vaccine_type created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_EHEALTHPORTAL_IMMUNISATION_VACCINE_TYPE_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_IMMUNISATION_VACCINE_TYPE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.immunisation_vaccine_type.'.$this->id);
			$this->setRules($rules);
		}
