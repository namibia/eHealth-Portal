...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Payment_typeTable.php
...
/**
 * Payment_types Table class
 */
class Payment_typeTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.payment_type';
		...
		parent::__construct('#__ehealthportal_payment_type', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Payment_type table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New payment_type. A payment_type created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			if ($table->load(['alias' => $this->alias]) && ($table->id != $this->id || $this->id == 0))
			{
				...
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_PAYMENT_TYPE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.payment_type.'.$this->id);
			$this->setRules($rules);
		}
