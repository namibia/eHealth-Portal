...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Diagnosis_typeTable.php
...
/**
 * Diagnosis_types Table class
 */
class Diagnosis_typeTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_ehealthportal.diagnosis_type';
		...
		parent::__construct('#__ehealthportal_diagnosis_type', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Diagnosis_type table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New diagnosis_type. A diagnosis_type created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_EHEALTHPORTAL_DIAGNOSIS_TYPE_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_EHEALTHPORTAL_DIAGNOSIS_TYPE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ehealthportal.diagnosis_type.'.$this->id);
			$this->setRules($rules);
		}
