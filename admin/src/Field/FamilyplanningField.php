...
/-------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		FamilyplanningField.php
...
/**
 * Familyplanning Form Field class for the Ehealthportal component
 */
class FamilyplanningField extends ListField
{
	/**
	...
	protected function getInput()
	{
		...
		if ($set_button === 'true')
		{
			...
			// only add if user allowed to create planning_type
			if ($user->authorise('core.create', 'com_ehealthportal') && $app->isClient('administrator')) // TODO for now only in admin area.
			{
				...
				$button[] = '<a id="'.$button_code_name.'Create" class="btn btn-small btn-success hasTooltip" title="'.Text::sprintf('COM_EHEALTHPORTAL_CREATE_NEW_S', $button_label).'" style="border-radius: 0px 4px 4px 0px; padding: 4px 4px 4px 7px;"
					href="index.php?option=com_ehealthportal&amp;view=planning_type&amp;layout=edit'.$ref.'" >
					<span class="icon-new icon-white"></span></a>';
			}
			// only add if user allowed to edit planning_type
				...
				$script[] = "
					...
					function ".$button_code_name."Button(value) {
						if (value > 0) {
							...
							var url = 'index.php?option=com_ehealthportal&view=planning_types&task=planning_type.edit&id='+value+'".$refJ."';
			...
			// check if button was created for planning_type field.
	...
	protected function getOptions()
	{
		...
		$query->from($db->quoteName('#__ehealthportal_planning_type', 'a'));
		$query->where($db->quoteName('a.published') . ' = 1');
		$query->order('a.name ASC');
		// Implement View Level Access (if set in table)
		if (!$user->authorise('core.options', 'com_ehealthportal'))
		{
			$columns = $db->getTableColumns('#__ehealthportal_planning_type');
		...
		if ($items)
		{
			...
			{
				$options[] = Html::_('select.option', $item->id, $item->diagnosis_name);
			}
		}
		return $options;
	}
