...
class Ehealth_portalViewImmunisation_vaccine_types extends JViewLegacy
{
	...
	protected function addToolBar()
	{
		...
		if ($this->canCreate)
		{
			JToolBarHelper::addNew('immunisation_vaccine_type.add');
		}
		...
		if (Ehealth_portalHelper::checkArray($this->items))
		{
			if ($this->canEdit)
			{
				JToolBarHelper::editList('immunisation_vaccine_type.edit');
			}

			if ($this->canState)
			{
				JToolBarHelper::publishList('immunisation_vaccine_types.publish');
				JToolBarHelper::unpublishList('immunisation_vaccine_types.unpublish');
				JToolBarHelper::archiveList('immunisation_vaccine_types.archive');

				if ($this->canDo->get('core.admin'))
				{
					JToolBarHelper::checkin('immunisation_vaccine_types.checkin');
				}
			}
			...
			{
				JToolbarHelper::deleteList('', 'immunisation_vaccine_types.delete', 'JTOOLBAR_EMPTY_TRASH');
			}
			elseif ($this->canState && $this->canDelete)
			{
				JToolbarHelper::trash('immunisation_vaccine_types.trash');
			}
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			$this->administration_partNameOptions = JFormHelper::loadFieldType('Administrationparts')->options;
			// We do some sanitation for Administration Part Name filter
			if (Ehealth_portalHelper::checkArray($this->administration_partNameOptions) &&
				isset($this->administration_partNameOptions[0]->value) &&
				!Ehealth_portalHelper::checkString($this->administration_partNameOptions[0]->value))
			{
				unset($this->administration_partNameOptions[0]);
			}
			// Administration Part Name Batch Selection
			JHtmlBatch_::addListSelection(
				'- Keep Original '.JText::_('COM_EHEALTH_PORTAL_IMMUNISATION_VACCINE_TYPE_ADMINISTRATION_PART_LABEL').' -',
				'batch[administration_part]',
				JHtml::_('select.options', $this->administration_partNameOptions, 'value', 'text')
			);
		}
	}
	...
	protected function getSortFields()
	{
		return array(
			...
			'g.name' => JText::_('COM_EHEALTH_PORTAL_IMMUNISATION_VACCINE_TYPE_ADMINISTRATION_PART_LABEL'),
