 ...
 * Payment View class
 */
class Ehealth_portalViewPayment extends JViewLegacy
{
	/**
	...
	public function display($tpl = null)
	{
		...
		$this->canDo = Ehealth_portalHelper::getActions('payment', $this->item);
		// get input
	...
	protected function addToolBar()
	{
		...
		JToolbarHelper::title( JText::_($isNew ? 'COM_EHEALTH_PORTAL_PAYMENT_NEW' : 'COM_EHEALTH_PORTAL_PAYMENT_EDIT'), 'pencil-2 article-add');
		...
		else
		{
			...
			else
			{
				...
				{
					JToolbarHelper::versions('com_ehealth_portal.payment', $this->item->id);
				}
				if ($this->canDo->get('core.create'))
				{
		...
		$help_url = Ehealth_portalHelper::getHelpUrl('payment');
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_($isNew ? 'COM_EHEALTH_PORTAL_PAYMENT_NEW' : 'COM_EHEALTH_PORTAL_PAYMENT_EDIT'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_ehealth_portal/assets/css/payment.css", (Ehealth_portalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
		...
		$this->document->addScript(JURI::root() . "administrator/components/com_ehealth_portal/views/payment/submitbutton.js", (Ehealth_portalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/javascript'); 
