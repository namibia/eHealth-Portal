 ...
 */
class Ehealth_portalViewFoetal_presentation extends JViewLegacy
{
	/**
	...
	public function display($tpl = null)
	{
		...
		$this->canDo = Ehealth_portalHelper::getActions('foetal_presentation', $this->item);
		// get input
	...
	protected function addToolBar()
	{
		...
		JToolbarHelper::title( JText::_($isNew ? 'COM_EHEALTH_PORTAL_FOETAL_PRESENTATION_NEW' : 'COM_EHEALTH_PORTAL_FOETAL_PRESENTATION_EDIT'), 'pencil-2 article-add');
		...
		else
		{
			...
			else
			{
				...
				{
					JToolbarHelper::versions('com_ehealth_portal.foetal_presentation', $this->item->id);
				}
				if ($this->canDo->get('core.create'))
				{
		...
		$help_url = Ehealth_portalHelper::getHelpUrl('foetal_presentation');
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_($isNew ? 'COM_EHEALTH_PORTAL_FOETAL_PRESENTATION_NEW' : 'COM_EHEALTH_PORTAL_FOETAL_PRESENTATION_EDIT'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_ehealth_portal/assets/css/foetal_presentation.css", (Ehealth_portalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
		...
		$this->document->addScript(JURI::root() . "administrator/components/com_ehealth_portal/views/foetal_presentation/submitbutton.js", (Ehealth_portalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/javascript'); 
