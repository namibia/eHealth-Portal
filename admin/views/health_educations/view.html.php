 ...
 */
class EhealthportalViewHealth_educations extends HtmlView
{
	/**
	...
	function display($tpl = null)
	{
		if ($this->getLayout() !== 'modal')
		{
			...
			EhealthportalHelper::addSubmenu('health_educations');
		}
	...
	{
		JHtmlSidebar::setAction('index.php?option=com_ehealthportal&view=health_educations');
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			$this->education_typeOptions = FormHelper::loadFieldType('healtheducationsfiltereducationtype')->options;
			// We do some sanitation for Education Type filter
			if (ArrayHelper::check($this->education_typeOptions) &&
				isset($this->education_typeOptions[0]->value) &&
				!StringHelper::check($this->education_typeOptions[0]->value))
			{
				unset($this->education_typeOptions[0]);
			}
			// Education Type Batch Selection
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_EHEALTHPORTAL_HEALTH_EDUCATION_EDUCATION_TYPE_LABEL').' -',
				'batch[education_type]',
				Html::_('select.options', $this->education_typeOptions, 'value', 'text')
			);
		}
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			$this->patientOptions = FormHelper::loadFieldType('healtheducationsfilterpatient')->options;
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_EHEALTHPORTAL_HEALTH_EDUCATION_PATIENT_LABEL').' -',
				'batch[patient]',
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(Text::_('COM_EHEALTHPORTAL_HEALTH_EDUCATIONS'));
