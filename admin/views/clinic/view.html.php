 ...
 * Clinic View class
 */
class Ehealth_portalViewClinic extends JViewLegacy
{
	/**
	...
	public function display($tpl = null)
	{
		...
		$this->canDo = Ehealth_portalHelper::getActions('clinic', $this->item);
		// get input
	...
	protected function addToolBar()
	{
		...
		JToolbarHelper::title( JText::_($isNew ? 'COM_EHEALTH_PORTAL_CLINIC_NEW' : 'COM_EHEALTH_PORTAL_CLINIC_EDIT'), 'pencil-2 article-add');
		...
		else
		{
			...
			else
			{
				...
				{
					JToolbarHelper::versions('com_ehealth_portal.clinic', $this->item->id);
				}
				if ($this->canDo->get('core.create'))
				{
		...
		$help_url = Ehealth_portalHelper::getHelpUrl('clinic');
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_($isNew ? 'COM_EHEALTH_PORTAL_CLINIC_NEW' : 'COM_EHEALTH_PORTAL_CLINIC_EDIT'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_ehealth_portal/assets/css/clinic.css", (Ehealth_portalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
		...
		$this->document->addScript(JURI::root() . "administrator/components/com_ehealth_portal/views/clinic/submitbutton.js", (Ehealth_portalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/javascript'); 
