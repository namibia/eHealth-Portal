...
/**
 * Ehealth_portal View class for the Breast_cancers
 */
class Ehealth_portalViewBreast_cancers extends JViewLegacy
{
	/**
	...
	function display($tpl = null)
	{
		if ($this->getLayout() !== 'modal')
		{
			...
			Ehealth_portalHelper::addSubmenu('breast_cancers');
		}
	...
	{
		JToolBarHelper::title(JText::_('COM_EHEALTH_PORTAL_BREAST_CANCERS'), 'heart');
		JHtmlSidebar::setAction('index.php?option=com_ehealth_portal&view=breast_cancers');
		...
		if (Ehealth_portalHelper::checkArray($this->items))
		{
			...
			{
				JToolBarHelper::custom('breast_cancers.exportData', 'download', '', 'COM_EHEALTH_PORTAL_EXPORT_DATA', true);
			}
		}

		if ($this->canDo->get('core.import') && $this->canDo->get('breast_cancer.import'))
		{
			JToolBarHelper::custom('breast_cancers.importData', 'upload', '', 'COM_EHEALTH_PORTAL_IMPORT_DATA', false);
		}

		// set help url for this view if found
		$help_url = Ehealth_portalHelper::getHelpUrl('breast_cancers');
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.JText::_('COM_EHEALTH_PORTAL_BREAST_CANCER_PATIENT_LABEL').' -',
				'batch[patient]',
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_('COM_EHEALTH_PORTAL_BREAST_CANCERS'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_ehealth_portal/assets/css/breast_cancers.css", (Ehealth_portalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
	}
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.patient' => JText::_('COM_EHEALTH_PORTAL_BREAST_CANCER_PATIENT_LABEL'),
