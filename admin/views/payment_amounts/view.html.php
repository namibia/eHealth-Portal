...
/**
 * Ehealth_portal View class for the Payment_amounts
 */
class Ehealth_portalViewPayment_amounts extends JViewLegacy
{
	/**
	...
	{
		JToolBarHelper::title(JText::_('COM_EHEALTH_PORTAL_PAYMENT_AMOUNTS'), 'joomla');
		JHtmlSidebar::setAction('index.php?option=com_ehealth_portal&view=payment_amounts');
		...
		if (Ehealth_portalHelper::checkArray($this->items))
		{
			...
			{
				JToolBarHelper::custom('payment_amounts.exportData', 'download', '', 'COM_EHEALTH_PORTAL_EXPORT_DATA', true);
			}
		}

		if ($this->canDo->get('core.import') && $this->canDo->get('payment_amount.import'))
		{
			JToolBarHelper::custom('payment_amounts.importData', 'upload', '', 'COM_EHEALTH_PORTAL_IMPORT_DATA', false);
		}

		// set help url for this view if found
		$help_url = Ehealth_portalHelper::getHelpUrl('payment_amounts');
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_('COM_EHEALTH_PORTAL_PAYMENT_AMOUNTS'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_ehealth_portal/assets/css/payment_amounts.css", (Ehealth_portalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
	}
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.name' => JText::_('COM_EHEALTH_PORTAL_PAYMENT_AMOUNT_NAME_LABEL'),
			'a.description' => JText::_('COM_EHEALTH_PORTAL_PAYMENT_AMOUNT_DESCRIPTION_LABEL'),
