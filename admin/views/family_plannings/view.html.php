...
/**
 * Ehealth_portal View class for the Family_plannings
 */
class Ehealth_portalViewFamily_plannings extends JViewLegacy
{
	/**
	...
	function display($tpl = null)
	{
		if ($this->getLayout() !== 'modal')
		{
			...
			Ehealth_portalHelper::addSubmenu('family_plannings');
		}
	...
	{
		JToolBarHelper::title(JText::_('COM_EHEALTH_PORTAL_FAMILY_PLANNINGS'), 'calendar-2');
		JHtmlSidebar::setAction('index.php?option=com_ehealth_portal&view=family_plannings');
		...
		if (Ehealth_portalHelper::checkArray($this->items))
		{
			...
			{
				JToolBarHelper::custom('family_plannings.exportData', 'download', '', 'COM_EHEALTH_PORTAL_EXPORT_DATA', true);
			}
		}

		if ($this->canDo->get('core.import') && $this->canDo->get('family_planning.import'))
		{
			JToolBarHelper::custom('family_plannings.importData', 'upload', '', 'COM_EHEALTH_PORTAL_IMPORT_DATA', false);
		}

		// set help url for this view if found
		$help_url = Ehealth_portalHelper::getHelpUrl('family_plannings');
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.JText::_('COM_EHEALTH_PORTAL_FAMILY_PLANNING_PATIENT_LABEL').' -',
				'batch[patient]',
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_('COM_EHEALTH_PORTAL_FAMILY_PLANNINGS'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_ehealth_portal/assets/css/family_plannings.css", (Ehealth_portalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
	}
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.patient' => JText::_('COM_EHEALTH_PORTAL_FAMILY_PLANNING_PATIENT_LABEL'),
			'g.name' => JText::_('COM_EHEALTH_PORTAL_FAMILY_PLANNING_DIAGNOSIS_LABEL'),
